<?php
abstract class Genesis_Dambuster_Admin {
	const METABOX_CLASS = 'genesis-dambuster-metabox';
   ...
   private $messages = array();
	...
	function __construct($version, $path, $parent_slug, $slug = '') {
		...
		$this->tooltips = new Genesis_Dambuster_Tooltip($this->tips);
 	...
 	function news_panel($post,$metabox){	
		Genesis_Dambuster_Feed_Widget::display_feeds(apply_filters('genesis_dambuster_newsfeeds', array( GENESIS_DAMBUSTER_NEWS )));
	}

   function make_icon($icon) {
		if (empty($icon)) $icon = GENESIS_DAMBUSTER_ICON;
		return sprintf('<img src="%1$s" alt="%2$s" />', $icon, GENESIS_DAMBUSTER_PLUGIN_NAME) ;
   }
   ...
   function is_metabox_active($post_type, $context) {
	...
	function display_metabox($tabs, $n=0) {
      ...
      printf('<div class="%1$s"><ul class="metabox-tabs">%2$s</ul><div class="metabox-content">%3$s</div><input type="hidden" class="tabselect" name="%4$s" value="%5$s" /></div>', self::METABOX_CLASS, $labels, $contents, $tabselect, $tab);
   }
