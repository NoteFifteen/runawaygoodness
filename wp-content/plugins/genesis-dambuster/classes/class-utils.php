...
class Genesis_Dambuster_Utils {
   ...
   function get_current_term() {
	...
	function hide_widget($visibility ) {
		$hide = false;
		$is_landing = $this->is_landing_page();
		switch ($visibility) {
	...
	function read_more_link($link_text='Read More', $class='') {
      $classes = empty($class) ? '' : (' ' . $class);
 		return sprintf('<a class="more-link%1$s" href="%2$s">%3$s</a>', $classes, get_permalink(), $link_text);
 	}
    ...
    function register_icons_font() {
        ...
        wp_enqueue_style('font-awesome-ie7', plugins_url('styles/font-awesome-ie7.min.css', dirname(__FILE__)), array(), '4.4.0', 'all');
