	...
	public static function generate_social_profile_icons($profiles = array(), $button_type = 'square', 
		...
		foreach ($profiles as $network => $address) {
			...
			if ($profiles_text) {
				...
				$output .= sprintf('<li class="essb-single-profile" %6$s><a href="%1$s" target="_blnak" rel="nofollow" class="essb-profile-all essb-profile-%2$s-%3$s" %5$s><span class="essb-profile-icon essb-profile-%2$s"></span>%4$s</a></li>', $address, $network, $button_fill, $text, $single_width, $button_width);
			}
			else {
				$output .= sprintf('<li class="essb-single-profile"><a href="%1$s" target="_blnak" rel="nofollow" class="essb-profile-all essb-profile-%2$s-%3$s"><span class="essb-profile-icon essb-profile-%2$s"></span></a></li>', $address, $network, $button_fill);
			}
		}
